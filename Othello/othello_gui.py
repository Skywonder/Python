...
class Gui():
    ...
    def game_Start(self, game):
        ...
        canvas_frame.grid(row = 0, column = 0, sticky = stickyAll)
        canvas_frame.rowconfigure(0, weight =1)
        canvas_frame.columnconfigure(0, weight =1)
        
        #create the starting canvas of the game
        self._canvas = tk.Canvas(master = canvas_frame, width= 800 , height= 700, background='green')
    ...
    def draw_line(self):
        ...
        minicanvas_height = height//game.getRow()
        minicanvas_width = width//game.getColumn()
        
        for i in range(1, game.getRow()):
            #draw horizontal line
            self._canvas.create_line(0, i * minicanvas_height, minicanvas_width* game.getColumn(), i * minicanvas_height)
            #draw vertical line

        for i in range(1, game.getColumn()):
            self._canvas.create_line(i * minicanvas_width, 0, i * minicanvas_width, minicanvas_height * game.getRow())
    ...
    def draw_board(self, board):
        ...
        for x in range(game.getRow()):
            for y in range(game.getColumn()):
                if board[x][y] == black:
                    board[x][y] = self._canvas.create_oval(y * minicanvas_width + 4, x*minicanvas_height + 4, (y+1)*minicanvas_width - 8,
                        (x+1)*minicanvas_height - 8, fill="black", outline ="#DDD", width = 2)

                elif board[x][y] == white:
                    board[x][y] = self._canvas.create_oval(y * minicanvas_width + 4, x*minicanvas_height + 4, (y+1)*minicanvas_width - 8, 
                        (x+1)*minicanvas_height - 8, fill="white", outline ="#DDD", width = 2)
        return board
    ...
    def _show_game_score(self):
        ...
        self.score_canvas1 = tk.Canvas(score_frame, width= minicanvas_width * game.getColumn()//2 , height= minicanvas_height * game.getRow()//4, background='white')
        ...
        self.score_canvas2 = tk.Canvas(score_frame, width= minicanvas_width * game.getColumn()//2 , height= minicanvas_height * game.getRow()//4, background='white')
    ...
    def on_click(self, event:tk.Event):
        ...
        self.point = (event.y//minicanvas_height, event.x//minicanvas_width)
        self.clicked = True
